...
export const SYSTEM_CONTRACT_ADDRESSES = new Set([
  '0x0000000000000000000000000000000000000000',
  // Somnia System Contract Addresses
  SOMNIA_ADDRESSES.operatorPermissionsRegistry.toLowerCase(),
  ...
  SOMNIA_ADDRESSES.oracleHub.toLowerCase(),
  SOMNIA_ADDRESSES.clobFactory.toLowerCase(),
  SOMNIA_ADDRESSES.operatorAccount.toLowerCase(),
]);

export const HARDHAT_TEST_ADDRESSES = new Set([
...
export function isExcludedArenaAddress(address?: string | null): boolean {
  if (!address) return true;
  const clean = address.trim().toLowerCase();
  if (SYSTEM_CONTRACT_ADDRESSES.has(clean)) return true;
  if (process.env.NODE_ENV !== 'test' && HARDHAT_TEST_ADDRESSES.has(clean)) return true;
  return false;
}
...
export class LeaderboardService {
  ...
  public async getTraderProfile(address: string): Promise<TraderProfileDetail | null> {
    if (isExcludedArenaAddress(address)) return null;
