...
<p align="center">
  ...
  <a href="https://github.com/zaikaman/DreamPulse"><img src="https://img.shields.io/badge/Tests-520%2F520%20Passing%20(100%25)-0284c7?style=for-the-badge&logo=vitest&logoColor=white" alt="Tests 520/520 Passing" /></a>
...
* **Live Verifiable Transaction Ledger (.txt)**: [https://dreampulse-backend-2aa35c1a9af1.herokuapp.com/transactions.txt](https://dreampulse-backend-2aa35c1a9af1.herokuapp.com/transactions.txt) *(Over 1,400 fills, over 100 tUSDC in profits, live ASCII export)*
...
* **DreamPulse Smart Account Factory (V2, current)**: [`0x8fa0e6ce3ed29df26ce6961ad22a7d51c273b4da`](https://shannon-explorer.somnia.network/address/0x8fa0e6ce3ed29df26ce6961ad22a7d51c273b4da) *(Deploys per-user EIP-1167 Smart Trading Account clones; source: `contracts/DreamPulseSessionAccountV2.sol`)*
...
* **Automated Verification Suite**: `npm run verify` *(520/520 Tests Passing — 428 backend + 92 frontend, 0 Typecheck Errors, CI coverage gates + testnet staging probe green)*
...
11. [Live Verifiable Swarm Execution Ledger (.txt)](#live-verifiable-swarm-execution-ledger-txt)
12. [Mathematical & Quantitative Foundation](#mathematical--quantitative-foundation)
13. [Non-Custodial Smart Trading Account & Security Architecture](#non-custodial-smart-trading-account--security-architecture)
14. [Institutional Design System & Minimalist UI](#institutional-design-system--minimalist-ui)
...
21. [Verification & Test Suite (520 Passing: 428 Backend + 92 Frontend)](#verification--test-suite-520-passing-428-backend--92-frontend)
...
> The canonical Protocol Swarm running live on Somnia Shannon Testnet (Chain ID `50312`) provides:
> * **Autonomous Multi-Agent Quoting**: Active limit quoting and opportunistic arbitrage across Volt, Oracle, and Titan.
> * **Automated Settlement Redemption**: Zero-loss capital recovery via the Sweeper daemon immediately upon market resolution.
> * **Real-Time Evaluation Latency**: Sub-100ms evaluation loops with an average evaluation latency of **`1ms`** (47ms tick).
> * **100% Verifiable & Non-Custodial**: View live analytics, active resting orders, and execution streams in the [Auditable Cockpit](https://dreampulse-ai.vercel.app/#cockpit).
...
* **Strategy**: Mathematical Implied vs. Realized Volatility Arbitrage with Multi-Layer Quantitative Protection.
* **Mechanism**: Continuously prices binary prediction event contracts using high-precision Black-Scholes standard normal cumulative distribution $\Phi(d_2)$. Compares theoretical fair value with the mid-market price on DreamDEX. When the net edge exceeds post-spread, post-fee thresholds ($\ge 3.5\%$), Oracle executes trades to exploit mispriced probability surfaces while strictly validating trend alignment.
* **Risk Invariants & 3-Layer Quantitative Protection**:
  * **Layer 1 (Horizon Filter)**: Restricts execution strictly to rapid-convergence windows ($45\text{s} \le T \le 900\text{s}$ / 15-minute maximum). Hard-blocks long-duration contracts ($> 900\text{s}$, such as 1h, 4h, and 24h) where diffusion assumption breakdown leaves un-hedged positions exposed to spot drift.
  * **Layer 2 (Multi-Timeframe Trend Gating)**: Evaluates real-time price action ($EMA_9$, $EMA_{21}$, $RSI_7$, $\Delta_{\text{1m}}$, $\Delta_{\text{5m}}$). Hard-blocks counter-trend fades: strictly rejects `BUY NO` when the underlying asset is in a `BULLISH_EXPANSION` or surging, and strictly rejects `BUY YES` during `BEARISH_BREAKDOWN` or sharp spot plunges.
  * **Layer 3 (Dynamic Asymmetry Margin-of-Safety Collar)**: Doubles required minimum EV edge to $\ge 7.0\%$ and ROI hurdle to $\ge 16.0\%$ (vs. standard $3.5\%$ and $8.0\%$) whenever taking mild counter-trend positions.
  ...
  * **Gamma pin-risk lockout**: Refuses execution when time remaining $< 45\text{s}$ or $> 900\text{s}$.
  * **Minimum Return-on-Capital hurdle**: Enforces baseline $E[\text{ROI}] \ge 8.0\%$, scaling to $\ge 16.0\%$ on asymmetric risk profiles.
...
To provide an open, transparent, and instantly verifiable paper trail for hackathon judges, quants, and auditors without requiring database access or web UI overhead, DreamPulse serves a live, paginated plain-text ASCII transaction ledger directly from the database pipeline:

* **Live Plain-Text Ledger Link**: [https://dreampulse-backend-2aa35c1a9af1.herokuapp.com/transactions.txt](https://dreampulse-backend-2aa35c1a9af1.herokuapp.com/transactions.txt) *(also accessible via `/api/v1/swarm/transactions.txt`)*

### Live Telemetry & Verification Features:
* **High-Altitude Aggregates**: Top-of-file summary displaying total on-chain executions, cumulative trading volume, overall win rate, net realized profits, and agent fill breakdowns across Volt, Oracle, Titan, and Sweeper.
* **Sub-5ms Fast-Path Response**: Built with non-blocking background settlement sync (`void syncResolvedOrdersPnLAsync()`) and dual-layer in-memory caching (2.0s aggregate cache, 1.5s rendered page cache), streaming plain text with zero RPC bottlenecks.
* **Full In-Memory Pagination**: Uncapped by PostgREST limits, allowing instant traversal across all 1,400+ transactions (`?page=1&limit=50`, `?page=2`, etc.) with retro-terminal monospace alignment, order IDs, timestamp (UTC), agent role, side, token direction (YES/NO), price, stake, status, and direct Somnia Shannon block explorer transaction links.
...
### 7. Quantitative Protection Architecture: 3-Layer Defense for Volatility Arbitrage
In continuous binary prediction markets, un-hedged Black-Scholes pricing suffers from **Drift-Diffusion Asymmetry** on long-duration contracts. Because prediction tokens cannot be continuously delta-hedged like TradFi vanilla options, pricing contracts with large horizons ($T > 900\text{s}$, e.g. 1-hour or 4-hour pools) using pure diffusion ($\sigma \sqrt{T}$) flattens theoretical probabilities $\Phi(d_2) \to 0.50$. When the underlying spot asset experiences a strong directional trend, the CLOB odds correctly skew (e.g. YES = 0.65, NO = 0.35). An unprotected model falsely diagnoses an apparent "edge" on the NO token ($0.50 - 0.35 = +15\%$), aggressively fading the trend and suffering systematic drawdowns.

To protect operator and copy-trading capital against macro drift, DreamPulse implements a battle-tested **3-Layer Quantitative Defense Architecture**:

```
                                  [ Candidate Mispricing Detected ]
                                                 │
                                 ┌───────────────┴───────────────┐
                                 ▼                               ▼
                      Layer 1: Horizon Filter          T > 900s or T < 45s? ────► [ REJECT ] (Hard Lockout)
                                 │
                                 ▼ (T in [45s, 900s])
                      Layer 2: Trend Gating            Counter-Trend in Expansion? ──► [ REJECT ] (Zero Fades)
                                 │
                                 ▼ (Neutral / Confluent)
                      Layer 3: Asymmetry Collar        Margin-of-Safety Met? ──► [ EXECUTE IOC ]
                                                       (7.0% Edge / 16% ROI if counter)
```

1. **Layer 1: Horizon Filter ($45\text{s} \le T \le 900\text{s}$)**:
   Restricts Oracle volatility arbitrage strictly to rapid-convergence windows ($\le 15$ minutes). Contracts exceeding 900 seconds (1-hour, 4-hour, 24-hour) are hard-blocked because realized drift dominates diffusion variance over multi-hour horizons.

2. **Layer 2: Multi-Timeframe Trend-Aware Gating**:
   Evaluates underlying spot microstructure across exponential moving averages ($EMA_9, EMA_{21}$), Relative Strength Index ($RSI_7$), and multi-window velocity ($\Delta_{\text{1m}}, \Delta_{\text{5m}}$):
   $$\text{Bullish Expansion} \iff (S_t > EMA_9 > EMA_{21}) \land (\Delta_{\text{5m}} > 0.05\% \lor RSI_7 > 62)$$
   $$\text{Bearish Breakdown} \iff (S_t < EMA_9 < EMA_{21}) \land (\Delta_{\text{5m}} < -0.05\% \lor RSI_7 < 38)$$
   * **Rule**: Hard-block `BUY NO` during Bullish Expansions or rapid upward spot spikes ($\Delta_{\text{1m}} > 0.12\%$).
   * **Rule**: Hard-block `BUY YES` during Bearish Breakdowns or rapid downward spot plunges ($\Delta_{\text{1m}} < -0.12\%$).

3. **Layer 3: Dynamic Asymmetry Margin-of-Safety Collar**:
   When a candidate order is non-expanding but mildly counter to prevailing 5-minute spot momentum ($\Delta_{\text{5m}} \cdot \text{direction} < 0$), the execution gateway enforces an expanded collar, doubling the required mathematical advantage:
   $$\text{minEdge}_{\text{effective}} = \max(\text{minEdge}_{\text{base}}, 7.0\%)$$
   $$\text{minROI}_{\text{effective}} = \max(\text{minROI}_{\text{base}}, 16.0\%)$$
   Trades failing this enhanced hurdle are pruned before submitting on-chain transactions, preserving swarm capital and avoiding unfavorable risk-reward entries.
...
| :--- | :--- | :--- | :--- |
| **`DreamPulseSessionAccountFactory` (V2, current)** | `0x8fa0e6ce3ed29df26ce6961ad22a7d51c273b4da` | Factory deploying per-user EIP-1167 Smart Trading Account clones (`contracts/DreamPulseSessionAccountV2.sol`, deployed 2026-09-08) | [View on Explorer](https://shannon-explorer.somnia.network/address/0x8fa0e6ce3ed29df26ce6961ad22a7d51c273b4da) |
| **`DreamPulseSessionAccount` V2 (Impl, current)** | `0xbb61bcec697edd6e6aed54a7d7d9a85fd5a899b7` | Smart account implementation with on-chain risk policies & pinned withdrawals | [View on Explorer](https://shannon-explorer.somnia.network/address/0xbb61bcec697edd6e6aed54a7d7d9a85fd5a899b7) |
...
| **Technical Implementation (25%)** | • Deep integration with `@somnia-chain/markets-sdk` across orders, depth ladders, cancellations, and settlement redemptions.<br />• **Battle-Tested On-Chain Performance**: Autonomous multi-agent pipeline executing with a sub-100ms loop and 1ms average evaluation latency on Somnia Shannon.<br />• 520/520 unit and integration tests passing (428 backend across 36 suites + 92 frontend across 10 suites) with strict TypeScript compilation (0 errors), CI-enforced money-path coverage gates, and a read-only testnet staging probe.<br />• Deployed dedicated per-user EIP-1167 Smart Trading Account clones (`DreamPulseSessionAccountFactory`) with on-chain risk policies and owner-pinned withdrawals, fully isolating trading collateral from primary wallets.<br />• Dynamic `NonceManager` handling sub-second on-chain concurrency and automated revert circuit breakers. |
...
## Verification & Test Suite (520 Passing: 428 Backend + 92 Frontend)
...
| 🔵 **Tested Locally** | **Automated Vitest Suite (Deterministic Mocks)** | • **Mathematical Invariants**: Closed-form Abramowitz-Stegun CDF polynomial approximation, Avellaneda-Stoikov inventory skew, Sharpe/Sortino ratios, integer quantization to 6-decimal micro-tUSDC.<br />• **Cryptographic & Non-Custodial Boundaries**: EIP-712 typed data hashing and signature recovery, session nonce tracking, single-trade risk ceilings ($500 limit), daily volume caps ($5,000 rolling cap), and max 30-day session durations verified without consuming testnet gas.<br />• **Resilience & Fallback Paths**: Controlled mock injection verifying that if Groq API keys return HTTP 401 or exhaust quotas, the cognitive engine falls back to deterministic quantitative math logs; and if GraphQL indexers lag, the backend polls direct on-chain contract state. |

---

### Comprehensive Test Suite Breakdown (520 Tests Across 46 Suites)
...
| :--- | :---: | :---: | :--- |
| [`tests/api.test.ts`](file:///d:/DreamPulse/backend/tests/api.test.ts) | **38** | 🔵 Local Integration | Express REST API health, market lists, order book depth ladders, anomaly feeds, telemetry stream endpoints, session management routes, order execution logs, copy-trade toggle, custom swarms, sweeper trigger, and formatted plain-text ASCII ledger export with pagination. |
...
| [`tests/order-service.test.ts`](file:///d:/DreamPulse/backend/tests/order-service.test.ts) | **34** | 🟢 Live / Local | User manual orders, autonomous agent executions, resting limit order lifecycle (PENDING, PARTIALLY_FILLED, CANCELLED, EXPIRED), partial fill accounting, direct user on-chain tx receipt verification, VOID/YES/NO market settlements, pagination, and PnL reconciliation. |
...
| [`tests/session.test.ts`](file:///d:/DreamPulse/backend/tests/session.test.ts) | **25** | 🔵 Local Unit | Non-custodial Smart Trading Account clone registration, dedicated per-user session keys, sequential nonce tracking, EIP-712 typed signature verification, on-chain risk caps ($500 single trade limit, $5,000 rolling 24h cap, 30-day max duration), session revocation, multi-wallet isolation, 24h rolling cap enforcement, and copy-trade target filtering. |
...
| [`tests/session-money-guard.test.ts`](file:///d:/DreamPulse/backend/tests/session-money-guard.test.ts) | **10** | 🔵 Local Unit | Adversarial spend-cap suite: expiry auto-deactivation, 24h window reset, exact cap-boundary semantics, nonce reuse, custodial-permission smuggling, 4-decimal rounding, hostile registrations, tx-hash provenance. |
| [`tests/settlement-money-guard.test.ts`](file:///d:/DreamPulse/backend/tests/settlement-money-guard.test.ts) | **6** | 🔵 Local Unit | Sweep idempotency (no double-count on re-record), PENDING/FAILED exclusion from totals, hex-alias merging, per-user/per-market isolation, history ordering and limits. |
| [`tests/order-money-guard.test.ts`](file:///d:/DreamPulse/backend/tests/order-money-guard.test.ts) | **13** | 🔵 Local Unit | Lot-size floor-never-rounds-up quantization, NO-complement pricing, dust-to-zero, SELL/VOID PnL signs, unknown-winner resolution, malformed-hash and attacker-contract forgery rejection. |
| [`tests/order-replay-race.test.ts`](file:///d:/DreamPulse/backend/tests/order-replay-race.test.ts) | **2** | 🔵 Local Integration | Concurrent double-submit with the same txHash executes exactly once (in-flight lock reserved synchronously); lock release verified, no deadlock. |
| [`tests/session-reservation.test.ts`](file:///d:/DreamPulse/backend/tests/session-reservation.test.ts) | **7** | 🔵 Local Unit | Atomic `reserveTradeSpend` holds (concurrent reserves cannot jointly overspend), TTL self-heal, consume-with-actual-fill true-up, explicit release, advisory-check visibility. |
| [`tests/operator-approval.test.ts`](file:///d:/DreamPulse/backend/tests/operator-approval.test.ts) | **6** | 🔵 Local Unit | Copy-trader global registry approval vs 100 tUSDC allowance threshold, dual-RPC failure, sync-throw fallback. |
...
| **Backend Total** | **428** | **All 3 Tiers** | **100% Passing across 36 suites with zero failures (Strict TypeScript)** |

Frontend — `npm run test --workspace=dreampulse-frontend` (Vitest + jsdom + Testing Library):

| Test File | Tests | What Is Verified |
| :--- | :---: | :--- |
| [`src/lib/confluence.test.ts`](file:///d:/DreamPulse/frontend/src/lib/confluence.test.ts) | **14** | `erf`/`normalCdf`/Black-Scholes binary probability math, sigmoid fallback, BUY_UP / BUY_DOWN / CAUTION_COUNTER_TREND confluence branches. |
| [`src/lib/errorUtils.test.ts`](file:///d:/DreamPulse/frontend/src/lib/errorUtils.test.ts) | **13** | BigInt/circular safe-stringify, EIP-1193 4001 rejection detection, balance/gas/network/wallet parsing, viem boilerplate stripping. |
| [`src/lib/sessionUtils.test.ts`](file:///d:/DreamPulse/frontend/src/lib/sessionUtils.test.ts) | **13** | Contract ceilings ($500 trade, $5,000 daily cap, 30-day max duration), cap formatting, session time-remaining strings, and expiry calculations. |
| [`src/lib/utils.test.ts`](file:///d:/DreamPulse/frontend/src/lib/utils.test.ts) | **9** | Tailwind class merging, currency/number/address formatting. |
| [`src/lib/navigation.test.ts`](file:///d:/DreamPulse/frontend/src/lib/navigation.test.ts) | **8** | Canonical view hashes, alias resolution, profile-address deep links. |
| [`src/lib/lazy-with-retry.test.ts`](file:///d:/DreamPulse/frontend/src/lib/lazy-with-retry.test.ts) | **8** | Chunk-load failure detection, transient retry with recovery, fatal-error fast path. |
| [`src/services/soundEngine.test.ts`](file:///d:/DreamPulse/frontend/src/services/soundEngine.test.ts) | **7** | Web Audio synthesizer initialization, gain node envelope shaping, audio toggle persistence, and procedural sound cue dispatch (order placed, filled, error, alert). |
| [`src/components/SessionDelegationModal.test.tsx`](file:///d:/DreamPulse/frontend/src/components/SessionDelegationModal.test.tsx) | **12** | Spend-approval surface: connect/network gating, contract-capped delegation params ($500 trade, $5,000 daily cap, 30-day duration), mirror-consent passthrough, two-click revoke with on-chain opt-out, error render/dismiss, signing lockout. |
| [`src/components/SessionStatusBar.test.tsx`](file:///d:/DreamPulse/frontend/src/components/SessionStatusBar.test.tsx) | **4** | Real-time session status bar indicator, active session duration formatting, non-custodial clone balance display, and direct revoke/manage modal triggers. |
...
| **Frontend Total** | **92** | **100% Passing across 10 suites with zero failures (Strict TypeScript)** |

| **Grand Total** | **520** | **100% Passing across 46 suites — `npm run verify` (typecheck + tests + production builds)** |
...
```
 ...
 ✓ tests/order-service.test.ts (34 tests)
 ✓ tests/ai-rate-limiter.test.ts (6 tests)
 ✓ tests/ledger-export.test.ts (3 tests)
 ✓ tests/api.test.ts (38 tests)
 ✓ tests/llm.test.ts (7 tests)
 ✓ tests/session-money-guard.test.ts (10 tests)
 ✓ tests/settlement-money-guard.test.ts (6 tests)
 ✓ tests/order-money-guard.test.ts (13 tests)
 ✓ tests/order-replay-race.test.ts (2 tests)
 ✓ tests/session-reservation.test.ts (7 tests)
 ✓ tests/operator-approval.test.ts (6 tests)
...
```
  ✓ src/lib/confluence.test.ts (14 tests)
  ✓ src/lib/navigation.test.ts (8 tests)
  ✓ src/lib/errorUtils.test.ts (13 tests)
  ✓ src/lib/polling.test.ts (4 tests)
  ✓ src/services/soundEngine.test.ts (7 tests)
  ✓ src/lib/utils.test.ts (9 tests)
  ✓ src/lib/sessionUtils.test.ts (13 tests)
  ✓ src/lib/lazy-with-retry.test.ts (8 tests)
  ✓ src/components/SessionStatusBar.test.tsx (4 tests)
  ✓ src/components/SessionDelegationModal.test.tsx (12 tests)

  Test Files  10 passed (10)
       Tests  92 passed (92)
```

CI additionally enforces V8 coverage thresholds, per-file money-path floors
(`coverage:gate`), and a read-only Somnia testnet staging probe
(`staging:dry-run`, 13/13 checks green).
---

## 2–3 Minute Demo Video Walkthrough

<p align="center">
  <a href="https://www.youtube.com/watch?v=Ld62mpVEb0U">
...
* **Direct Video URL**: [https://www.youtube.com/watch?v=Ld62mpVEb0U](https://www.youtube.com/watch?v=Ld62mpVEb0U)
